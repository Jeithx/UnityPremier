...
public class TimelineGrid : MonoBehaviour
{
    ...
    private void UpdateTimeRuler()
    {
        ...
        float[] tickIntervals = { 0.1f, 0.5f, 1f, 2f, 5f, 10f, 30f, 60f, 300f };
        float targetPixelInterval = 50f;
        float currentPixelPerSecond = config.gridCellHorizontalPixelCount / 4f; // 4 sec = gridCell

        float bestInterval = 1f;
        foreach (float interval in tickIntervals)
        {
            if (interval * currentPixelPerSecond >= targetPixelInterval)
            {
        ...
        {
            CreateTimeMarker(time, bestInterval);
        }
    }

    private void CreateTimeMarker(float timeSeconds, float interval)
    {
        float xPos = TimeToX(timeSeconds);


        bool isMajorTick = IsIntervalMultiple(timeSeconds, interval * 5f) || timeSeconds == 0;
        ...
        float tickWidth = isMajorTick ? 2f : 1f;
        float tickHeight = isMajorTick ? config.rulerHeight * 0.8f : config.rulerHeight * 0.4f;

        tickRT.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, tickWidth);
        tickRT.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, tickHeight);
    ...
    {
        GameObject textGO = new GameObject($"TimeText_{timeSeconds:F1}s", typeof(RectTransform), typeof(Text));
        RectTransform textRT = textGO.GetComponent<RectTransform>();
        textRT.SetParent(timeRulerRT, false);

        textRT.anchorMin = new Vector2(0, 0.6f);
        textRT.anchorMax = new Vector2(0, 1f);
        textRT.pivot = new Vector2(0.5f, 0.5f);
        textRT.anchoredPosition = new Vector2(xPos, 0);
        textRT.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal, 60f);
        textRT.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical, config.rulerHeight * 0.4f);
